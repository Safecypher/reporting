...
Plan: —
Status: Awaiting next milestone
Last activity: 2026-09-23 - Completed quick task 260923-max: collapsible Settings group in the sidebar footer
...
| 260923-ili | Multi-file upload on /uploads — the dropzone was hard-limited to one file (`multiple: false`, `acceptedFiles[0]`), so loading the ~25-report TSYS XLSX backlog meant 25 separate drag-drop-wait cycles. Now accepts a multi-file drop and uploads sequentially (one request in flight, never `Promise.all`) against the UNCHANGED single-file `/api/ingest` — so per-file sha256 de-dup, the 5 MB cap, auth and the 13 Aug 2026 window all still apply per file. Continue-on-failure is structural, not incidental: `uploadOne()` is a total function returning a `BatchFileOutcome` on every path (ok / !ok / thrown / unparsable JSON), so nothing can escape the loop and short-circuit the batch. New `lib/upload/batch.ts` holds the pure model and ALL user-visible copy (23 Vitest tests); `batch-results.tsx` renders the per-file list reusing `UploadResult` unchanged. Adds `n of N` progress, one batch summary line, ONE toast, ONE `router.refresh()`. Scope addition: accept-filter rejections previously vanished silently — invisible at 1 file, a real hazard at 1 of 25 — and now surface as `skipped` rows. Client layer only; ingest route, `lib/ingestion/**` and `upload-result.tsx` byte-identical. Tests 471→494, lint 0 errors/13 warnings (baseline). Live browser drag-drop UAT still outstanding | 2026-09-23 | 6b7d082 | [260923-ili-allow-multi-file-upload-on-the-uploads-d](./quick/260923-ili-allow-multi-file-upload-on-the-uploads-d/) |
| 260923-max | Settings navigation moved out of the main sidebar list into a collapsible `Settings` group pinned in the footer above Sign out. `/settings/general` and `/settings/pricing` sat mid-list between SLA and Reconciliation — configuration surfaces presented as peers of the reporting pages. New `components/app-shell/settings-nav.tsx` renders one cog-icon `Settings` row whose `Collapsible.Trigger asChild` wraps `SidebarMenuButton` (no `asChild` on the button — that is what keeps the trigger a real `<button>` carrying Radix aria-expanded/aria-controls, rather than an anchor). Auto-expand on `/settings/*` uses `key={inSettings ? ... : ...}` + `defaultOpen` rather than a `useEffect` state sync, so direct URL load, back/forward and in-app navigation all land expanded while a manual toggle survives moving between non-settings pages. Chevron is `#arrow-right` rotated 90° via `group-data-[state=open]/collapsible:rotate-90` with a `motion-reduce` guard — deliberately NO new sprite symbol, since a symbol lacking the Lucide stroke defaults renders as a solid black blob (the 260914-ot4 bug). `Collapsible` imported from the `radix-ui` umbrella, not `@radix-ui/react-collapsible` (transitive only — would be a phantom dependency); no dependency added. Nav/shell layer only: sprite, `package.json` and `sign-out-button.tsx` byte-identical. tsc clean, lint 0 errors/13 warnings, 494 tests, build 17 routes — all baseline. 8-step browser UAT (keyboard operation, accessible name, aria-expanded flip, mobile Sheet) outstanding | 2026-09-23 | 626388b | [260923-max-move-general-and-pricing-out-of-the-main](./quick/260923-max-move-general-and-pricing-out-of-the-main/) |
